...
displayH g = S.Translate width2 height2 $ case _status g of
    ...
    where -- paddle = drawItem (S.Color 0 127 0 1) (_paddle g)
          (px, py) = _pos $ _paddle g
          paddle = S.Translate px py $ S.Image S.Original $ _img g
...
main = do
    ...
    myImg <- S.makeImage "spongebob2.png"
    -- TODO audio
    -- myAudio <- newAudioContext
    let model0 = createGame myRands myImg
    S.play ctx doc 30 model0 displayH eventH idleH

-- https://github.com/ghcjs/ghcjs-dom/tree/master/ghcjs-dom-jsffi/src/GHCJS/DOM/JSFFI/Generated
