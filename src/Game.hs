...
import qualified Graphics.Shine.Image as S
...
createGame :: (Fractional a, Ord a) => [a] -> S.ImageData-> Game a
createGame rands0 img = Game Running False False False rands1 img 0 myPaddle [] myInvaders
    where myPaddle = Item (70, 66) (0, -250) (0, 0)
          -- myPaddle = Item (70, 20) (0, -250) (0, 0)
...
updatePaddle :: (Fractional a, Ord a) => a -> Game a -> Game a
...
fireInvadersBullets :: (Fractional a, Ord a) => Game a -> Game a
...
step time g = if _status g == Running then mRunning else mEnd
    where mRunning = updatePaddle time
                        ...
                        $ updateBullets time
                        $ updateCollisions g
          mEnd = if _inputFire g then createGame (_rands g) (_img g) else g
